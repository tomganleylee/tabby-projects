            ...
            <div class="ps-main">
                ...
                <ng-container *ngIf="sel.kind === 'project'">
                    ...
                    <table class="table table-sm ps-tabs">
                        <tr><th class="w-40px"></th><th>Title</th><th class="w-100px">Kind</th><th>Command</th><th title="Run instead of Command when the tab comes back after a Tabby restart">After restart</th><th class="w-60px" title="Open when the project opens">Auto</th><th class="w-60px" title="Let the program name the tab (Claude Code shows its conversation summary)">Live name</th><th class="w-40px"></th></tr>
                        <tr *ngFor="let t of sel.project.tabs; let i = index">
                            ...
                            <td><input type="checkbox" class="form-check-input" [disabled]="t.kind !== 'shell'" [ngModel]="t.dynamicTitle ?? isClaude(t)" (ngModelChange)="t.dynamicTitle = $event; save()"></td>
                            <td><button class="btn btn-sm btn-link" (click)="sel.project.tabs.splice(i, 1); save()" [innerHTML]="ui.trash | safeHtml"></button></td>
                        </tr>
                        <tr *ngIf="iconRow"><td colspan="8"><icon-picker [value]="iconRow.icon" (valueChange)="iconRow.icon = $event; save()"></icon-picker></td></tr>
                    </table>
...
export class ProjectsSettingsComponent implements OnInit {
    ...
    isClaude (t: ProjectTabSpec): boolean {
        return /claude/i.test(t.command ?? '')
    }
